...
def log_prediction(data: dict):
    ...
    with open(filename, mode='a', newline='', encoding='utf-8') as f:
        fieldnames = [
            'timestamp', 'symbol', 'interval', 'signal', 'price',
            'confidence', 'accuracy', 'raw_prediction', 'stop_loss', 'take_profit'
        ]
...
def add_core_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["fake_breakout"] = (
            (df["high"] > rolling_high_20) & (df["close"] < df["open"])
    ).astype(int)
...
def build_aux_features(df: pd.DataFrame, prefix: str) -> pd.DataFrame:
    ...
    aux = aux.rename(
        columns={
            ...
            "EMA_cross": f"{prefix}_EMA_cross",
            "trend_strength": f"{prefix}_trend_strength",
            "price_vs_ema20": f"{prefix}_price_vs_ema20",
...
def merge_multi_timeframe_features(
    ...
    for aux_interval in aux_intervals:
        ...
        if aux_raw is None or len(aux_raw) < 50:
            continue
...
def build_feature_list(df: pd.DataFrame):
    features = [
        "RSI", "RSI_slope", "MACD", "MACD_sig", "MACD_hist", "EMA_cross",
        "BB_bandwidth", "BB_percent", "OBV_signal", "ATR_pct", "Stoch_K",
        "Stoch_D", "Stoch_cross", "body_size", "upper_wick", "lower_wick",
        "vol_ratio", "volume_spike", "momentum_3", "momentum_7",
        "trend_strength", "price_vs_ema20", "fake_breakout",
        "RSI_lag1", "RSI_lag2", "MACD_hist_lag1", "MACD_hist_lag2",
        "price_vs_ema20_lag1", "price_vs_ema20_lag2", "Stoch_cross_lag1", "Stoch_cross_lag2"
    ]

    mtf_candidates = [
        "5m_RSI", "5m_MACD_hist", "5m_EMA_cross", "5m_trend_strength", "5m_price_vs_ema20", "5m_ATR_pct",
        "15m_RSI", "15m_MACD_hist", "15m_EMA_cross", "15m_trend_strength", "15m_price_vs_ema20", "15m_ATR_pct",
        "1h_RSI", "1h_MACD_hist", "1h_EMA_cross", "1h_trend_strength", "1h_price_vs_ema20", "1h_ATR_pct",
        "4h_RSI", "4h_MACD_hist", "4h_EMA_cross", "4h_trend_strength", "4h_price_vs_ema20", "4h_ATR_pct",
        "1d_RSI", "1d_MACD_hist", "1d_EMA_cross", "1d_trend_strength", "1d_price_vs_ema20", "1d_ATR_pct",
        "1w_RSI", "1w_MACD_hist", "1w_EMA_cross", "1w_trend_strength", "1w_price_vs_ema20", "1w_ATR_pct",
        "1M_RSI", "1M_MACD_hist", "1M_EMA_cross", "1M_trend_strength", "1M_price_vs_ema20", "1M_ATR_pct",
    ]
...
def train_and_evaluate(X_train, y_train, X_pred, features, future_bars, actual_fetch_interval):
    ...
    avg_accuracy = round(float(np.mean(cv_accuracy)) * 100, 1)
    avg_precision = round(float(np.mean(cv_precision)) * 100, 1)
    avg_recall = round(float(np.mean(cv_recall)) * 100, 1)
    avg_f1 = round(float(np.mean(cv_f1)) * 100, 1)
...
def get_ml_signal(
        symbol: str,
        interval: str = "4h",
        min_confidence: float = 51.0,
        min_accuracy: float = 50.1,
) -> dict:
    binance_sym = resolve_binance_symbol(symbol)

    if interval == "1M":
        actual_fetch_interval = "1w"
        future_bars = 4
        atr_target_mult = 0.3
    elif interval == "1w":
        actual_fetch_interval = "1d"
        future_bars = 5
        atr_target_mult = 0.4
    elif interval == "1d":
        actual_fetch_interval = "1h"
        future_bars = 12
        atr_target_mult = 0.5
    elif interval == "4h":
        actual_fetch_interval = "15m"
        future_bars = 8
        atr_target_mult = 0.5
    elif interval == "1h":
        actual_fetch_interval = "5m"
        future_bars = 6
        atr_target_mult = 0.5
    else:
        actual_fetch_interval = interval
        future_bars = 3
        atr_target_mult = 0.3
    ...
    if last_features_row.isna().any():
        missing_cols = last_features_row[last_features_row.isna()].index.tolist()
        return {
            "status": "error",
            "reason": f"Останній рядок містить NaN у фічах: {missing_cols}",
        }
    ...
    if len(df_strict) >= min_bars_required - future_bars:
        ...
        mode = "fallback_binary"
        df_train = df_fallback
        target_col = "target_fallback"
    else:
        return {
            "status": "error",
            "reason": "Недостатньо розмічених даних для бектесту",
        }
    ...
    if confidence < min_confidence or avg_accuracy < min_accuracy:
        final_signal = "NO TRADE ⚪"
    else:
        final_signal = "LONG 🟢" if prediction == 1 else "SHORT 🔴"
    ...
    time_multiplier = np.sqrt(future_bars)
    scaled_atr = atr * time_multiplier

    max_tp_pct = 0.12
    max_sl_pct = 0.06

    raw_sl_dist = scaled_atr * 1.0
    raw_tp_dist = scaled_atr * 2.0

    actual_sl_dist = min(raw_sl_dist, price * max_sl_pct)
    actual_tp_dist = min(raw_tp_dist, price * max_tp_pct)

    if final_signal == "LONG 🟢":
        stop_loss = price - actual_sl_dist
        take_profit = price + actual_tp_dist
    ...
    result = {
        ...
        "interval": interval,
        "signal": final_signal,
        "raw_prediction": "LONG" if prediction == 1 else "SHORT",
        "mode": mode,
        "confidence": confidence,
        ...
        "price": round(price, 6),
        "rsi": round(float(last_raw["RSI"]), 1) if pd.notna(last_raw["RSI"]) else None,
