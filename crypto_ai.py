...
def extract_coin_from_question(text: str):
    """
    Витягує тікер монети з питання користувача.
    Наприклад:
    - "яка ціна btc" -> BTC
    - "що по solana" -> SOL
    - "ефір росте?" -> ETH
...
def get_realtime_binance_price(symbol: str):
    """
    Бере актуальну ціну з Binance.
    TTL 15 секунд, щоб ціна не зависала надовго в кеші.
    """
    if not symbol:
        return None
...
def build_realtime_price_context(user_question: str):
    """
    Якщо користувач питає про конкретну монету,
    додаємо в AI-промпт актуальні дані по цій монеті.
    """
...
st.markdown(
    f'<div class="hero-sub">// Binance + CoinGecko + NewsAPI + Groq AI ({MODEL_NAME})</div>',
    unsafe_allow_html=True
)
...
if user_ticker and (
    ...
    <div class="tradingview-widget-container" style="height:100%;width:100%; border-radius:16px; overflow:hidden; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 1.5rem; position: relative; box-shadow: 0 20px 70px rgba(131,72,193,0.10), 0 8px 25px rgba(0,0,0,0.35);">
      ...
      new TradingView.widget({{
        ...
        "interval": "{tv_interval_map.get(selected_interval, "240")}", 
...
""", unsafe_allow_html=True)
    ...
    st.markdown(
        '<div class="hint" style="cursor: default;">↵ Enter — надіслати повідомлення</div>',
        unsafe_allow_html=True
    )
    ...
    if os.path.exists("predictions_log.csv"):
        with st.expander("📜 Журнал логів (predictions_log.csv)"):
            try:
                st.dataframe(
                    pd.read_csv("predictions_log.csv").tail(10).iloc[::-1],
