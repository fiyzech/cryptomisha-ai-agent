...
/* Приховуємо стандартний UI Streamlit */
...
def build_analysis_prompt(name, symbol, price, change_24h, change_7d, cap, vol, ath, low_24h, high_24h, spark_info, news):
    ...
    return f"""Ти — Pulse-AI, досвідчений крипто-наставник і аналітик. Пиши чистою українською.
Стиль: як досвідчений трейдер пояснює учню — чітко, структуровано, зрозуміло навіть для новачка.
...
Напиши структурований огляд:
1. Що зараз відбувається з монетою (2-3 речення)
2. На що звернути увагу трейдеру (ключові рівні, тренд)
3. Загальний настрій ринку по цій монеті

Пояснюй терміни простими словами якщо використовуєш їх. Не давай прямих фінансових порад."""
...
def build_chat_prompt(name, symbol, price, change_24h, cap, vol, change_7d, news, history, user_q):
    ...
    return f"""Ти — Pulse-AI, крипто-наставник і фінансовий ментор.

Твоя роль:
- Ти як досвідчений трейдер і вчитель в одній особі
- Пояснюєш складні речі простою мовою — зрозуміло навіть для новачка
- Даєш практичні поради по стратегії, управлінню ризиками, психології трейдингу
- Відповідаєш на питання про крипту, блокчейн, DeFi, NFT, торгівлю, інвестиції
- Можеш відповідати і на загальні питання, якщо користувач запитує
- Якщо питають "що робити з монетою" — пояснюєш фактори, а не просто "купуй/продавай"
- Не вигадуєш ціни, відсотки або новини — лише реальні дані нижче

Стиль: українська мова, живо і по-людськи, без зайвого формалізму, не повторюй привітання щоразу.

Контекст активної монети: {name} ({symbol})
- Ціна: {fmt_price(price)}
- Зміна 24г: {change_24h:+.2f}%
{realtime_context}
Новини: {news_text}
...
if user_ticker and (analyze_btn or (st.session_state.current_coin == user_ticker and st.session_state.get("ml_result"))):
    ...
    if data:
        ...
        if realtime_selected:
            price = realtime_selected.get("price", price)
            change_24h = realtime_selected.get("change_24h", change_24h)
            low_24h = realtime_selected.get("low_24h", low_24h)
            high_24h = realtime_selected.get("high_24h", high_24h)
            vol = realtime_selected.get("volume", vol)
        ...
        price = realtime_selected.get("price", 0)
        change_24h = realtime_selected.get("change_24h", 0)
        low_24h = realtime_selected.get("low_24h", 0)
        high_24h = realtime_selected.get("high_24h", 0)
        vol = realtime_selected.get("volume", 0)
    ...
    if ml_result.get("status") == "success":
        ...
        raw_prediction_html = f'<div class="ml-acc">Сирий прогноз: <b>{ml_result.get("raw_prediction", "—")}</b> &nbsp;·&nbsp; Режим: <b>{ml_result.get("mode", "—")}</b></div>'

        sltp_html = ""
        if ml_result.get("stop_loss") is not None and ml_result.get("take_profit") is not None:
            sltp_html = (
                f'<div class="ml-acc">'
                f'<span data-tooltip="Stop Loss: Ціна, де угода закриється в мінус">🛑 SL: <b>{safe_price(ml_result.get("stop_loss"))}</b></span> '
                f'&nbsp;·&nbsp; '
                f'<span data-tooltip="Take Profit: Ціна, де угода закриється в плюс">🎯 TP: <b>{safe_price(ml_result.get("take_profit"))}</b></span> '
                f'&nbsp;·&nbsp; '
                f'<span data-tooltip="Співвідношення Ризик/Прибуток">R/R: <b>{ml_result.get("risk_reward", "2.0")}</b></span>'
                f'</div>'
            )
